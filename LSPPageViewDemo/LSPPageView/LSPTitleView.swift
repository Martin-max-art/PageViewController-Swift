...
protocol LSPTitleViewDeleate : class {
    func titleView(_ titleView : LSPTitleView, didSelected currentIndex : Int)
}

class LSPTitleView: UIView {
    
    // MARK: 定义属性
    weak var delegate : LSPTitleViewDeleate?
    
    fileprivate var titles : [String]
    fileprivate var style : LSPPageStyle
    ...
    // MARK: 构造函数
    init(frame : CGRect, titles : [String], style : LSPPageStyle) {
    ...
    private func setupTitleLabelsFrame() {
        ...
        for (i, label) in titleLabels.enumerated() {
            ...
            if !style.isScrollEnable {
                ...
                w = (titles[i] as NSString).boundingRect(with: CGSize(width : CGFloat.greatestFiniteMagnitude, height: 0), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : style.titleFont], context: nil).width
                if i == 0 {
...
extension LSPTitleView : LSPContentViewDelegate {
    func contentView(_ contentView: LSPContentView, endScroll inIndex: Int) {
        // 1.取出两个Label
        let oldLabel = titleLabels[currentIndex]
    ...
    func contentView(_ contentView: LSPContentView, targetIndex: Int, progress: CGFloat) {
        // 1.取出两个Label
        let oldLabel = titleLabels[currentIndex]
